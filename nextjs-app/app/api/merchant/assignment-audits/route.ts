...
export async function GET(request: NextRequest) {
  try {
    ...
    const consumerUserId = request.nextUrl.searchParams.get('consumerUserId') || undefined
    const status = request.nextUrl.searchParams.get('status') || undefined
    const limit = request.nextUrl.searchParams.get('limit') || undefined

    const path = withQuery('/merchant/assignment-audits', {
      merchant_user_id: merchantUserId,
      consumer_user_id: consumerUserId,
      status,
      limit,
    })
    const response = await backendApiFetch<{ assignments: unknown[] }>(path, { method: 'GET' }, undefined, authToken)
    ...
    const message = error instanceof Error ? error.message : 'Failed to load assignment audits'
